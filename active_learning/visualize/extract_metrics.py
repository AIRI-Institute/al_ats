...
def extract_info_from_config(config):
    ...
    data["acquisition"] = config["acquisition_model"].get("name") or config["acquisition_model"].get("checkpoint")
    data["successor"] = (
        config["successor_model"].get("name") or config["successor_model"].get("checkpoint")
        if config["successor_model"]
        else data["acquisition"]
    )
    data["target"] = (
        config["target_model"].get("name") or config["target_model"].get("checkpoint")
    ...
    data["framework"] = (
        config["framework"] if isinstance(config["framework"], str) else config["framework"]["name"]
    )
    return data
