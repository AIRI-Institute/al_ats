...
def cache_all(config, start_word="Caching", end_word="cached"):
    ...
    if config.data.path == "datasets":
        ...
        kwargs = {}
        if config.data.get("revision_master"):
            kwargs["revision"] = "master"
        if isinstance(config.data.dataset_name, str):
            load_dataset(config.data.dataset_name, cache_dir=data_cache_dir, **kwargs)
            log.info(
                "=" * 10
            ...
            load_dataset(*list(config.data.dataset_name), cache_dir=data_cache_dir, **kwargs)
            log.info(
                "=" * 10
